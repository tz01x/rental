...
function App() {
  const ak="";
  const api_server_url = window.location.host=="localhost"?"http://127.0.0.1:8000/":(window.location.protocol+"//"+window.location.host+ (window.location.port==""?'/':":"+window.location.port+"/"));
  ...
  async function load_fatch_Data(storage_key,url,state,setState,f,featch_from_api=false,get_raw_data=false){
  ...
  useEffect(() => {
    ...
    async function fetchdata() {
      ...
      await load_fatch_Data('citys','api/city/',allcitys,setAllcitys,(element)=>{
        allcitys.push({ val: element.name, inlineTxt: element.name });
        setAllcitys([...allcitys]);
      });
      await load_fatch_Data('ptyps','api/property/types/list/',allpTypes,setAllpTypes,(element)=>{
        allpTypes.push({ val: element.name, inlineTxt: element.name });
        setAllpTypes([...allpTypes]);
      });

      await load_fatch_Data('features','api/property/features/list/',allfeatures,setAllfeatures,(element)=>{
        allfeatures.push(element.name)
        setAllfeatures([...allfeatures]);
      });

      await load_fatch_Data('propertylist','api/property/list/',myproperty,setMyProperty,(element)=>{
      ...
      <div className="search_bar sbkimg">
        ...
        <div className="ifields">
          <ThemeProvider theme={theme}>
            ...
            <SimpleSelect key={"8k89jdjod7533#"} name="bedroom" label="Bed rooms" handelChangeFdata={handelChangeFdata} curr_value={fdata.bedroom} op={[0].map(() => {
              const d = [];
              for (let i = 1; i < 5; i++) {
            ...
            <SimpleSelect key={"8k898755k3#"} name="bathroom" label="Bath rooms" handelChangeFdata={handelChangeFdata} curr_value={fdata.bathroom} op={[0].map(() => {
              const d = [];
              for (let i = 1; i < 5; i++) {
                d.push({ val: i, inlineTxt: i });
              }
            ...
            < MultipleSelect name={"features"} label={"Features"} data={allfeatures} handelChangeFdata={handelChangeFdata} selectedFeature={fdata.features} />
          </ThemeProvider>
                  ...
                  <div className="card-body">
                    <div className="card-text">
                      ...
                      <small>
                        ...
                        <span id="bathnumber" className="pe-2">{property.bathroom} Baths</span><i className="fa fa-th-large"></i>
                        <span id="area" className="pe-2">{property.property_size} sqft</span>

                      </small>

                    </div>
                  </div>
                  <div className="card-footer" style={{ backgroundColor: "#ffff" }}>
                    <small className="text-muted fw-bold">BDT <span className="fs-5" style={{ color: "#ff653d" }}>
