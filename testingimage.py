...
import django 
django.setup()
from user.models import Citys,Districts,Thana
import json 
Citys.objects.all().delete()
Districts.objects.all().delete()
Thana.objects.all().delete()
divisions=json.load(open('division.json',encoding="utf8"))[0]['divisions']
...
for division in divisions:
    Citys.objects.create(name=division['name'],bn_name=division['bn_name'],lat=division['lat'],lon=division['long'])
last_div_id=None
city=None
division_name=None
